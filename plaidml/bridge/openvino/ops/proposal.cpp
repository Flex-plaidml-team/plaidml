...
edsl::Tensor enumerate_proposals(edsl::Tensor class_probs,                //
  ...
  auto feat_h_t = edsl::reshape(edsl::index({dim_h, dim_w, dim_anchor}, 0), {num_proposals, 1});
  auto feat_w_t = edsl::reshape(edsl::index({dim_h, dim_w, dim_anchor}, 1), {num_proposals, 1});
  auto img_x = edsl::cast((swap_xy ? feat_h_t : feat_w_t) * attrs.feat_stride, dtype);
  auto img_y = edsl::cast((swap_xy ? feat_w_t : feat_h_t) * attrs.feat_stride, dtype);
